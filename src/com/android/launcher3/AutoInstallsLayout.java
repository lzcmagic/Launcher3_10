...
public class AutoInstallsLayout {
     ...
     * add by luozc
     */
    public synchronized int loadOtherLayout() {

        LauncherApps mApps = (LauncherApps) mContext.getSystemService(Context.LAUNCHER_APPS_SERVICE);
        List<LauncherActivityInfo> activityList = mApps.getActivityList(null, UserHandle.SYSTEM);
        int count =0;
        int screenId = 1;
        int x = 0;
        int y = 0;
        ...
        for (LauncherActivityInfo info:activityList) {
            mValues.clear();
            mValues.put(Favorites.CONTAINER, -100);
            mValues.put(Favorites.SCREEN, screenId);

            mValues.put(Favorites.CELLX, x);
            mValues.put(Favorites.CELLY, y);
            ...
            final Intent intent = mPackageManager.getLaunchIntentForPackage(info.getComponentName().getPackageName());
            if (intent == null) {
                continue;
            }
            ...
            int i = mCallback.insertAndCheck(mDb, mValues);
    ...
    protected int parseLayout(XmlPullParser parser, IntArray screenIds)
        ...
        while (((type = parser.next()) != XmlPullParser.END_TAG ||
            ...
            System.out.println("tag name="+name+";depth="+depth1);
